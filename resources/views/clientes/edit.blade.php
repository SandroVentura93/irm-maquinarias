...
<div class="p-6 max-w-3xl mx-auto">
    ...
    <div class="grid grid-cols-2 gap-4">
            <div>
                ...
                <input name="nombre" value="{{ old('nombre', $cliente->nombre) }}" class="w-full border rounded p-2" required>
            </div>
            <div>
                <label class="block text-sm">Apellido</label>
                <input name="apellido" value="{{ old('apellido', $cliente->apellido) }}" class="w-full border rounded p-2">
            </div>
            <div>
                <label class="block text-sm">DNI</label>
                <input name="dni" value="{{ old('dni', $cliente->dni) }}" class="w-full border rounded p-2">
            </div>
            <div>
                <label class="block text-sm">RUC</label>
                <input name="ruc" value="{{ old('ruc', $cliente->ruc) }}" class="w-full border rounded p-2">
            </div>
            <div>
                <label class="block text-sm">Teléfono</label>
                <input name="telefono" value="{{ old('telefono', $cliente->telefono) }}" class="w-full border rounded p-2">
            </div>
            <div>
                <label class="block text-sm">Email</label>
                <input name="email" value="{{ old('email', $cliente->email) }}" class="w-full border rounded p-2" type="email">
            </div>
            <div>
                ...
                <select id="regionSelect" name="region_id" class="w-full border rounded p-2">
                    ...
                    @foreach($regiones as $r)
                        <option value="{{ $r->id }}" {{ $cliente->region_id == $r->id ? 'selected' : '' }}>{{ $r->nombre ?? $r->descripcion ?? $r->nombre_reg }}</option>
                    @endforeach
                </select>
            </div>
            <div>
                ...
                <select id="provinciaSelect" name="provincia_id" class="w-full border rounded p-2">
                    ...
                    @foreach($provincias as $p)
                        <option value="{{ $p->id }}" {{ $cliente->provincia_id == $p->id ? 'selected' : '' }}>{{ $p->nombre ?? $p->descripcion ?? $p->nombre_prov }}</option>
                    @endforeach
                </select>
            </div>
            <div>
                ...
                <select id="distritoSelect" name="distrito_id" class="w-full border rounded p-2">
                    ...
                    @foreach($distritos as $d)
                        <option value="{{ $d->id }}" {{ $cliente->distrito_id == $d->id ? 'selected' : '' }}>{{ $d->nombre ?? $d->descripcion ?? $d->nombre_dist }}</option>
                    @endforeach
                </select>
            </div>
        </div>
